		...
		<div class="container verticle-center">
			<div class="main-content">
				...
				<section class="link-section">
					...
					<button class="short-url-button noselect" data-clipboard-text="<?php echo $shorturl; ?>">Copia</button>
					<span class="info">Estatísticas <a href="<?php echo $shorturl; ?>+"><?php echo $url; ?>+</a></span>
				</section>
			</div>
	</section>
				...
				<?php if ( isset( $_REQUEST['url'] ) && $_REQUEST['url'] != 'http://' ): ?>
					<?php if (strpos($message,'added') === false): ?>
						<div id="error" class="alert alert-warning error" role="alert">
							<h5>Oh não, <?php echo $message; ?>!</h5>
						</div>	    
					<?php endif; ?>
				<?php endif; ?>
				<form id="shortenlink" method="post" action="">
					<input type="url" name="url" class="url" id="url" placeholder="COLE AQUI A URL" required>
					<input type="submit" value="Encurtar">
					<?php if (enableCustomURL): ?>
						<span class="customise-button noselect" id="customise-toggle"><img src="<?php echo siteURL ?>/frontend/assets/svg/custom-url.svg" alt="Options"> Personalizar</span>
						<div class="customise-container" id="customise-link" style="display:none;">
							<span><?php echo preg_replace("(^https?://)", "", siteURL ); ?>/</span>
							<input type="text" name="keyword" class="custom" placeholder="CUSTOMIZAR URL">
						</div>
					<?php endif; ?>
				</form>
			</section>
